...
VENDEDORA_IDS = [
    uid.strip() for uid in os.getenv("VENDEDORA_IDS", "123456789").split(",")
]
...
async def send_welcome(message: types.Message, state: FSMContext):
    ...
    if message.chat.type in ["group", "supergroup"]:
        if str(message.from_user.id) in VENDEDORA_IDS:
...
async def admin_panel(message: types.Message, state: FSMContext):
    if str(message.from_user.id) not in VENDEDORA_IDS:
